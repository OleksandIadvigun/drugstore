...
class StoreServiceTest @Autowired constructor(
    ...
    fun `should return received products`() {
        ...
        // when
        val transferCertificate = transactionTemplate.execute {
            storeService.returnProducts(1)
        } ?: fail("result is expected")

        // then
        assertThat(transferCertificate.id).isNotNull
        assertThat(transferCertificate.status).isEqualTo(TransferStatusDTO.RETURN)
        assertThat(transferCertificate.invoiceId).isEqualTo(1)
    }
    ...
    @Test
    fun `should not return products if status not received or delivered`() {
        ...
        // then
        assertThat(exception.message).contains("Invoice is not even PAID")
    }
}

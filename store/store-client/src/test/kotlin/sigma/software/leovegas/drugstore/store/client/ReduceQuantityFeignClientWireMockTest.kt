...
internal class ReduceQuantityFeignClientWireMockTestApp

@DisplayName("Reduce Store Item Quantity Feign Client WireMock test")
@ContextConfiguration(classes = [ReduceQuantityFeignClientWireMockTestApp::class])
class ReduceQuantityFeignClientWireMockTest @Autowired constructor(
    ...
    fun `should reduce store items quantity`() {
        ...
        // when
        val responseActual = storeClient.reduceQuantity(request)
        ...
        assertThat(responseActual[0].quantity).isEqualTo(5) // 10-5=15
    }
}
